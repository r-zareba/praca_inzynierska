...
model = Model(64, n_timestamps, n_features=x_train.shape[-1])
...
x_normal = np.concatenate((x_train, x_train))
y_pred = model.predict(x_normal)

loss = model.reconstruction_loss(x_normal)
anomaly_threshold = model.calculate_anomaly_threshold(x_normal)

fig, (ax1, ax2, ax3) = plt.subplots(3, sharex=True)

ax1.plot(x_normal[:, 0, 0], label='True i(t)', alpha=0.7)
ax1.plot(y_pred[:, 0, 0], label='Predicted i(t)')
ax1.legend(loc='upper right')

ax2.plot(x_normal[:, 0, 1], label='True u(t)', alpha=0.7)
ax2.plot(y_pred[:, 0, 1], label='Predicted u(t)')
ax2.legend(loc='upper right')

ax3.plot(loss)
ax3.axhline(y=anomaly_threshold, label='Reconstruction loss', c='r', linestyle='dashed')
...
#
# x_real = np.array(Xs)
# real_loss = model.reconstruction_loss(x_real)
# #
# fig, (ax1, ax2, ax3) = plt.subplots(3, sharex=True)
# ax1.plot(voltage)
# ax1.set_title('u(t)')
